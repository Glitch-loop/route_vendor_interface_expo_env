...
export default class DetermineIfInventoryOperationCancelableUseCase {
  ...
  async execute(id_inventory_operation: string): Promise<boolean> {
    ...
    const [inventoryOperation] = await this.unitOfWork.execute(
      async (repo) => {
    ...
    const index: number = dayOperations.findIndex(
      (dayOp) => dayOp.id_item === id_inventory_operation,
    );
    ...
    if (
      ...
      id_inventory_operation_type === DAY_OPERATIONS.end_shift_inventory
    ) {
      isCancelable = true;
    } else if (
