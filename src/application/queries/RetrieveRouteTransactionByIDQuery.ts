...
@injectable()
export default class RetrieveRouteTransactionByIDQuery {
  constructor(
  ...
  async execute(ids: string[]): Promise<RouteTransactionDTO[]> {
    const transactions: RouteTransaction[] = await this.routeTxRepo.retrieveRouteTransactionById(ids);
